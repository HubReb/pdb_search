...
- **FR-006**: The CLI entry point MUST be implemented using a mainstream CLI framework, replacing bespoke `argparse` + manual dialog loop. Subcommands MUST exist for every existing feature (search / add / update / delete / import / migrate). The interactive top-level menu MUST preserve the original four entries verbatim (search / add / update / quit) — adding more is a UX-surface change not authorised by this spec.
