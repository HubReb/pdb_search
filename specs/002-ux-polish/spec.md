...
- Q: Non-interactive update surface — what does `--id` mean for `pdbsearch update`? → A: `--id <N>` skips only the search step; table, field, and new-value prompts remain interactive. No `--table`/`--field`/`--value` flags are added (zero scripted callers exist today; can be added non-breakingly later if a real scripting need surfaces).
- Q: Who renders the alias-marking parens (e.g. `(P)apers`)? → A: The `ask_choice` helper auto-wraps the derived alias character inside the rendered label. Callers continue to pass plain labels (`"papers"`, `"abort"`); the helper handles both alias derivation *and* alias rendering. A caller may override by passing a pre-wrapped label together with an explicit alias for that option.
- Q: Confirmation summary text after a search-then-update pick on `papers` — title, id, or both? → A: Both. The confirmation echoes the paper's title *and* its id, e.g. `Please verify: You wish to change 'title' of the paper 'Direct speech-to-speech translation' (id 42) to '<new>'.` The id is retained for traceability (logs, post-hoc reconstruction); the title closes the user's recognition loop. Applies only to the `papers`-table search-then-update path; the legacy `bib`/`authors` raw-id paths retain their existing confirmation wording (the user-typed identifier is the only canonical handle there).
...
4. **Given** scripted use, **When** `pdbsearch update --id 42` is invoked, **Then** the search step is bypassed and the user proceeds directly into the existing table/field/value/confirmation prompt sequence with the row identified by `42`. (No `--table`/`--field`/`--value` flags are added in this feature; the surface is `--id`-only.)
...
- **FR-004**: Menu rendering MUST mark the alias character on each option label so the shortcut is discoverable, e.g. `1) (P)apers`, `4) (q)uit`. The rendering convention is "the alias character wrapped in parentheses inside the label" (matching the existing top-level `(Q)uit` precedent), and the wrap is applied **by the helper, not by the caller**: callers pass plain labels (`"papers"`, `"abort"`); `ask_choice` derives the alias per FR-002 and inserts the parens into the rendered output. A caller MAY override by passing a pre-wrapped label together with an explicit alias for that option (e.g. for a label whose first alphabetic char is not the desired alias).
...
- **FR-010**: `pdbsearch update` MUST accept a `--id <N>` flag that bypasses the search step on the `papers` table. Table, field, and new-value collection remain interactive in this mode (no `--table`/`--field`/`--value` flags are added in this feature). The flag is additive: invoking `update` without it preserves the new search-then-update flow end-to-end.
- **FR-011**: The confirmation prompt before the write MUST keep its current grammar (`1`/`y`/`yes` / `2`/`n`/`no`). On the `papers`-table search-then-update path, the summary line MUST echo both the picked paper's title and its id, in the form `You wish to change '<field>' of the paper '<title>' (id <N>) to '<new value>'.`, so the user verifies what they recognised in the disambiguation list while the id remains visible for log/audit traceability. On the legacy `bib`/`authors` raw-id paths the existing summary wording is preserved verbatim (the typed identifier is already the canonical handle).
...
- **SC-004**: Existing scripted callers using `pdbsearch delete --id <N>` keep working with zero edits, verified by retaining and re-running every existing integration test that exercises a non-interactive path. The new `pdbsearch update --id <N>` flag adds an additional scripted entry point on the `papers` table without removing or altering any existing path.
