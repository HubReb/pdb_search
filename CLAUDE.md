...
1. **`cli/`** — Typer subcommands (`search`, `add`, `update`, `delete`, `migrate`, `importer`, plus `app.py` that wires them and drops into the four-option top-level menu when invoked with no subcommand; `migrate` and `importer` are subcommand-only and deliberately absent from the menu — admin/scripted operations). `cli/prompts.py` is the only module under `src/paper_sorts/` permitted to read user input (`rich.prompt` / `input`), per constitution Principle III.
...
`migrations/versions/` holds the Alembic schema migrations. Revision 001 is the verbatim port of the legacy canonical DDL; revision 002 converges the legacy `bibtext_id` typo column onto the canonical schema (idempotent).
...
**Feature**: `specs/001-modernize-stack/` — Modernize the Stack. See [plan.md](specs/001-modernize-stack/plan.md), [research.md](specs/001-modernize-stack/research.md), [data-model.md](specs/001-modernize-stack/data-model.md), [contracts/cli-commands.md](specs/001-modernize-stack/contracts/cli-commands.md), and [quickstart.md](specs/001-modernize-stack/quickstart.md).
