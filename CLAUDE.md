...
`migrations/versions/` holds the Alembic schema migrations. Revision 001 is the verbatim port of the original DDL; future revisions converge legacy database variants (e.g. the `bibtext_id` typo column) onto the canonical schema.

For a reverse-engineered description of the *legacy* stack as it was before T026 (useful when reading historical commits or interpreting `seed_papers.py`'s edge-case coverage), see `docs/architecture.md`.
...
Session fixtures live in `tests/conftest.py` (`postgresql_proc`, `ephemeral_db_url`); the canonical seed dataset is `tests/fixtures/seed_papers.SEED_PAPERS`. Bench harness lives under `tests/benchmarks/`; `bench_baseline.py` is currently `@pytest.mark.skip`'d pending T046's modern rewrite (the recorded baseline lives in `tests/benchmarks/baseline.json`).

## SpecKit

`.specify/` contains SpecKit templates and `memory/constitution.md` (ratified 2026-04-26, current v1.4.0). The constitution defines four binding principles — Code Quality, Testing Standards, User Experience Consistency, Performance Requirements — and rules out a few things that come up naturally (mocking psycopg in DB tests; adding connection pools/caches/async drivers). The performance principle is framed as "no measurable regression vs. the current baseline" rather than absolute numbers — there's no benchmark behind any specific bound, so refactors are evaluated against measured baseline. Read the constitution before generating a plan or making non-trivial changes.

<!-- SPECKIT START -->
**Active feature**: `specs/002-ux-polish/` — UX Polish: Letter Aliases & Search-then-Act. See [plan.md](specs/002-ux-polish/plan.md) for the implementation plan, [research.md](specs/002-ux-polish/research.md) for the design decisions (alias-collision policy, helper-promotion location, signature evolution, call-site catalogue), [data-model.md](specs/002-ux-polish/data-model.md) (no schema changes; in-memory `MenuOption` only), [contracts/cli-grammar.md](specs/002-ux-polish/contracts/cli-grammar.md) for the `ask_choice` contract and the delta to apply to `specs/001-modernize-stack/contracts/cli-commands.md`, and [quickstart.md](specs/002-ux-polish/quickstart.md) for what's new for the user. Work on this feature happens on branch `002-ux-polish`. Predecessor 001-modernize-stack is closed (merged to main, branch deleted).
<!-- SPECKIT END -->
