...
For a reverse-engineered description of the *legacy* stack (useful when reading historical commits or interpreting the original codebase), see `docs/architecture.md`.
...
Session fixtures live in `tests/conftest.py` (`postgresql_proc`, `ephemeral_db_url`); the canonical seed dataset is `tests/fixtures/seed_papers.py`. Bench harness lives under `tests/benchmarks/`.

## SpecKit

<!-- SPECKIT START -->
**Active feature**: `specs/001-modernize-stack/` — Modernize the Stack (complete). See [plan.md](specs/001-modernize-stack/plan.md) for the implementation plan, [research.md](specs/001-modernize-stack/research.md) for design decisions, [data-model.md](specs/001-modernize-stack/data-model.md), [contracts/cli-commands.md](specs/001-modernize-stack/contracts/cli-commands.md), and [quickstart.md](specs/001-modernize-stack/quickstart.md).
<!-- SPECKIT END -->

`.specify/` contains SpecKit templates and `memory/constitution.md` (ratified 2026-04-26, current v1.3.0). The constitution defines four binding principles — Code Quality, Testing Standards, User Experience Consistency, Performance Requirements — and rules out a few things that come up naturally (mocking the SQLAlchemy session/repositories in DB tests; adding connection pools/caches/async drivers). The performance principle is framed as "no measurable regression vs. the current baseline" rather than absolute numbers — there's no benchmark behind any specific bound, so refactors are evaluated against measured baseline. Read the constitution before generating a plan or making non-trivial changes.
