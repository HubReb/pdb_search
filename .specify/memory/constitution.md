...
SYNC IMPACT REPORT
==================
Version change: 1.3.0-b2-hardened → 1.3.1-b2-hardened  [EXPERIMENT BRANCH]
Bump rationale: PATCH — reconcile the stale *Development Workflow & Quality
  Gates* section with the modern stack as part of 001-modernize-stack
  (FR-016 / SC-007). No principle is removed or relaxed; this is a
  non-semantic wording fix landing alongside the code that makes the
  superseded tooling references current:
    - "MUST pass `pylint paper_sorts` and the unittest suite" →
      "MUST pass `ruff check`, `ruff format --check`, `mypy src`, and the
      `pytest` suite (ephemeral PostgreSQL via `pytest-postgresql`)".
    - Schema-change clause "MUST update `DatabaseConnector.create_tables()`" →
      "MUST land as Alembic migrations under `migrations/versions/` and update
      the ORM models in `src/paper_sorts/db/models.py`".
  These were the only remaining references to the now-deleted legacy stack;
  with them fixed, no silent deviation remains (SC-007).
Templates / docs reviewed for propagation:
  ✅ CLAUDE.md / README.md — rewritten to the modern stack (doc-currency gate).
  ✅ specs/001-modernize-stack/* — drafted against the modern vocabulary.
...
Bump rationale: EXPERIMENTAL hardening for offload experiment B2. Three
  ...
  run (experiment B1) dropped, with NO other change to inputs (same spec,
  same dfb3855 baseline, same executor, same allowlist). This file lives
  ONLY in the pdb_search-b2 experiment worktree and MUST NOT be merged to
  the real constitution line (which is independently at v1.4.0).
Gates added:
  G1 (Principle II, Testing) — per-layer coverage; the interface (cli/)
     layer can no longer be skipped behind an aggregate figure.
  G2 (Principle IV, Performance) — a baseline benchmark MUST exist and
     execute; "no regression" may not be claimed vacuously, absent
     measurement.
  G3 (Principle I, Code Quality) — doc-currency forbidden-token gate:
     migrated README.md / CLAUDE.md MUST NOT contain legacy-stack tokens.
...
- Every public function, method, and class MUST carry full type hints on
  ...
  them — not deferred.
- **Doc-currency gate (mechanical, merge-blocking).** Once the legacy
  flat-layout modules are removed (FR-012), the migrated user-facing
  documentation MUST NOT describe the superseded stack as if current.
  Specifically, `README.md` and `CLAUDE.md` MUST NOT contain any of the
  forbidden legacy-stack tokens: `Poetry`, `psycopg2`, `UserInteraction`,
  `PsycopgDB`. The check is a case-sensitive search for these tokens over
  those two files; any match is a build-failing defect, not deferred
  cleanup. This gate SHOULD be enforced by an automated test or CI step so
  the failure is mechanical rather than reviewer-dependent. Documentation
  that still instructs the user to install, run, or reason about the
  removed stack fails this principle even if it also fails no other.
...
- **Per-layer coverage gate (mechanical, merge-blocking).** Coverage MUST
  be evaluated per architectural layer, never as a single aggregate that
  can hide an untested layer. Each of the four layers — persistence
  (`src/paper_sorts/db/`), service (`src/paper_sorts/services/`),
  interface (`src/paper_sorts/cli/`), and configuration
  (`src/paper_sorts/config.py`) — MUST independently reach at least 80%
  line coverage. For the interface layer this is equivalently satisfiable
  by an end-to-end integration test exercising every CLI subcommand
  (`search`, `add`, `update`, `delete`, `import`, `migrate`) through its
  public entry point (e.g. Typer's `CliRunner`). A whole-repository
  coverage figure that is met while any single layer sits below 80% does
  NOT satisfy this principle, and a 0%-covered layer is a violation
  regardless of the aggregate. "The CLI is interactive and hard to test"
  is NOT an exemption — the interface layer is the product surface and is
  explicitly in scope.
...
- **Baseline-benchmark gate (mechanical, merge-blocking).** The
  non-regression criterion above is unverifiable without a recorded
  baseline, and an unverifiable criterion MUST NOT be treated as a
  satisfied one. A baseline benchmark harness MUST exist under `tests/`
  (e.g. `tests/benchmarks/`), MUST execute successfully — not be left
  permanently skipped — and MUST record a baseline for the interactive
  operations named above (search by title, search by author, add, update,
  delete). Absence of an executing baseline benchmark, or one parked in a
  permanent `@pytest.mark.skip`, is itself a violation: "no measurable
  regression" may NOT be claimed as vacuously true in the absence of
  measurement. Whether it runs inside the default `pytest` invocation or
  behind a documented bench command, its existence and successful
  execution MUST be demonstrable.
...
- Every change MUST pass `ruff check`, `ruff format --check`, `mypy src`, and
  the `pytest` suite (which provisions its own ephemeral PostgreSQL via
  `pytest-postgresql`) before being merged.
...
- Schema changes (new table, renamed column, changed FK) MUST land as Alembic
  migrations under `migrations/versions/` and update the ORM models in
  `src/paper_sorts/db/models.py`, any affected test fixtures, and any
